...
app.get("/chk-email", function (req, resp) {
    ...
    dbCon.query("select * from products.signup where email=?", [req.query.kuchEmail], function (err, resultTable) {
      if (err == null) {
...
app.get("/chk-submit", function (req, resp) {
    ...
    dbCon.query("insert into products.signup(email,password,dos) values(?,?,current_date())", [req.query.kuchemail, req.query.kuchpwd], function (err) {
      if (err == null) {
    ...
    const options = {
      from: "savour.official2024@gmail.com", // sender ahddress
  ...
  app.get("/chk-login-submit", function (req, resp) {
    dbCon.query("select * from products.signup where email=? && password=?", [req.query.kuchemail, req.query.kuchpwd], function (err, resultJSONTable) {
      if (err == null) {
            resp.send("Done");
      }
      else {
  ...
  app.get("/get-items", function (req, resp) {
    //fixed                             //same seq. as in table
    dbCon.query("select distinct Food_type from dataset_final", function (err, resultTableJSON) {
      if (err == null)
  ...
  app.get("/fetch-donors", function (req, resp) {
    ...
    var query = "select * from dataset_final   where  dataset_final.Food_type=? Order by Price ASC  ";
  ...
  app.get("/submit", function (req, resp) {
    ...
    dbCon.query("insert into cart(Food_type,Restaurant,Price,Avg_rating,Delivery_partner) values(?,?,?,?,?)", [req.query.kuchemail, req.query.kuchmed, req.query.kuchquantity, req.query.kuchpacking, req.query.kuchdoe], function (err) {
      if (err == null) {
        resp.send("Record Saved successfully")
      }
      else {
  ...
  app.post("/submit-process", function (req, resp) {
    ...
    dbCon.query("insert into profile values(?,?,?,?,?,?,?)", [emailid, name, contact, address, city, id, fileName], function (err) {
      if (err == null) {
  ...
  app.get("/chk-cart", function (req, resp) {
    ...
    dbCon.query("insert into cart(ID) values(?)", [req.query.kuchID], function (err) {
      if (err == null) {
        resp.send("Record Saved successfully")
        transporter.sendMail(options, function (err, info) {
          if (err) {
  ...
  app.get("/get-angular-all-records", function (req, resp) {
    dbCon.query("SELECT * FROM dataset_final INNER JOIN cart ON dataset_final.ID = cart.ID ORDER BY dataset_final.ID ASC", function (err, resultTableJSON) {
      if (err == null)
        resp.send(resultTableJSON);
      else
        resp.send(err);
  ...
  app.get("/json-record", function (req, resp) {
    //fixed                             //same seq. as in table
    dbCon.query("select * from profile where emailid=?", [req.query.kuchemail], function (err, resultJSONKuch) {
      if (err == null) {
